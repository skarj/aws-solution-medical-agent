...
| **AWS Lambda** | Event-driven microservices for S3 event routing, rule parsing, patient full-text document consolidation (`patient-text-consolidator`, new 2026-08-27), the full-document reasoning invocation handler (`patient-screening-handler`), API handlers, and Bedrock Agent actions. | `[REQ-F-05]`, `[REQ-F-11]`, `[REQ-F-14]`, `[REQ-F-15]`, `[REQ-SEC-04]` | [AWS Lambda Documentation](https://docs.aws.amazon.com/lambda/) |
...
| **Amazon Bedrock (Agents)** | Single-agent deterministic reasoning engine, powered by Anthropic Claude Sonnet 5 (mandated by `[REQ-F-14]`, 2026-08-25), evaluating the complete consolidated patient document passed directly by `patient-screening-handler` (full-document deterministic reasoning, 2026-08-27 — no RAG/retrieval). Invoked via the Geographic (US) cross-Region inference profile `us.anthropic.claude-sonnet-5` per the `[REQ-OPS-01]` relaxation; verified 1,000,000-token context window comfortably holds a full patient record set. | `[REQ-F-14]`, `[REQ-F-15]`, `[REQ-F-16]` | [Amazon Bedrock Agents Documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/agents.html) |
| **Amazon Bedrock (Foundation Models)** | High-accuracy LLM reasoning for rule structuring (`[REQ-F-05]`) and full-document agentic verdict generation (`[REQ-F-14]`) — both mandate Anthropic Claude Sonnet 5, invoked via the Geographic (US) cross-Region inference profile per the 2026-08-25 `[REQ-OPS-01]` relaxation. Model card verified 2026-08-27: 1,000,000-token context window, 128K max output. | `[REQ-F-05]`, `[REQ-F-14]`, `[REQ-F-16]` | [Amazon Bedrock Foundation Models Documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html) |
| ~~Amazon Bedrock (Knowledge Bases)~~ | ~~Managed RAG ingestion pipeline providing chunking, vector embedding, and hybrid retrieval.~~ **Removed 2026-08-27** — superseded by full-document deterministic reasoning (`[REQ-F-11]` revised); no longer part of the architecture. | `[REQ-F-11]` (superseded) | — |
| ~~Amazon Titan Text Embeddings~~ | ~~Foundation model generating vector embeddings for patient document text chunks.~~ **Removed 2026-08-27** — no embeddings are generated under full-document deterministic reasoning. | `[REQ-F-11]` (superseded) | — |
...
| AWS Service | Architecture Role | Governing Requirements | Official Documentation |
| :--- | :--- | :--- | :--- |
| **Amazon Simple Storage Service (Amazon S3)** | Encrypted object storage for protocol PDFs, raw patient records, per-file extracted OCR text, and — as of 2026-08-27 — the consolidated per-patient full-text document (`patient-consolidated-text`) used for full-document deterministic reasoning. | `[REQ-F-01]`, `[REQ-F-04]`, `[REQ-F-07]`, `[REQ-F-10]`, `[REQ-F-11]`, `[REQ-SEC-01]` | [Amazon S3 Documentation](https://docs.aws.amazon.com/s3/) |
