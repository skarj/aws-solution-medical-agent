...
| **AWS Lambda** | Event-driven microservices for rule parsing, the explicit patient-screening trigger handler (`screening-trigger-handler`), patient full-text document consolidation (`patient-text-consolidator`), the full-document reasoning and citation-verification handler (`patient-screening-handler`, `[REQ-NF-06]`), and API handlers. | `[REQ-F-05]`, `[REQ-F-07]`, `[REQ-F-08]`, `[REQ-F-11]`, `[REQ-F-14]`, `[REQ-F-15]`, `[REQ-SEC-04]` | [AWS Lambda Documentation](https://docs.aws.amazon.com/lambda/) |
| **Amazon API Gateway** | Managed REST API edge providing authenticated Clinical Investigator access, presigned URL generation, and the explicit `POST /patients/{PatientID}/screenings` finalize-upload route that starts patient screening (replacing a raw per-file S3 event trigger to guarantee one execution per complete upload). | `[REQ-F-07]`, `[REQ-F-08]`, `[REQ-F-18]`, `[REQ-SEC-02]`, `[REQ-SEC-04]` | [Amazon API Gateway Documentation](https://docs.aws.amazon.com/apigateway/) |
| **Amazon EventBridge** | Serverless event bus routing protocol document upload events to the Protocol Onboarding Step Functions execution (Workflow 1 only; patient screening is explicitly triggered via API Gateway, not a raw S3 event, per `[REQ-F-07, REQ-F-08]`). | `[REQ-F-02]` | [Amazon EventBridge Documentation](https://docs.aws.amazon.com/eventbridge/) |
...
| **Amazon Bedrock Runtime (`InvokeModel`)** | Deterministic reasoning engine, powered by Anthropic Claude Sonnet 5 (mandated by `[REQ-F-14]`), evaluating the complete consolidated patient document passed directly by `patient-screening-handler` — full-document deterministic reasoning, no RAG/retrieval, and no Bedrock Agent wrapper (the screening step uses no Agent capability, so `[REQ-F-14]` specifies a direct `InvokeModel` call). Invoked via the Geographic (US) cross-Region inference profile `us.anthropic.claude-sonnet-5` per the `[REQ-OPS-01]` relaxation; verified 1,000,000-token context window comfortably holds a full patient record set. | `[REQ-F-14]`, `[REQ-F-15]`, `[REQ-F-16]` | [Amazon Bedrock Runtime InvokeModel API Reference](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_InvokeModel.html) |
...
| **Amazon DynamoDB** | Serverless NoSQL database storing structured study protocols (`study-protocols`) and patient verdicts (`patient-verdicts`); verdict writes from `patient-screening-handler` are conditional to protect an already-reviewed record from being overwritten by an automated re-run. | `[REQ-F-06]`, `[REQ-F-17]`, `[REQ-F-20]`, `[REQ-SEC-01]`, `[REQ-SEC-05]` | [Amazon DynamoDB Documentation](https://docs.aws.amazon.com/dynamodb/) |
