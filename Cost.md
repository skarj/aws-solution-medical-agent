...
* **Budget Status: Compliant, but margin is tight.** The baseline (low) figure is **23.7% under** the `[REQ-COST-01]` ceiling; the high end of the estimation range is only **1.8% under** it. **Flagged risk:** at $409.26/mo high end, normal month-to-month volume variance could push actual spend over the `[REQ-COST-01]` ceiling; recommend confirming actual token usage against Cost Explorer soon after production launch.
...
* **Estimated Monthly Subtotal:** **$27.50 – $30.25 / month**.
...
**Note:** AI Reasoning ($27.88–$30.67/mo) is Claude Sonnet 5 protocol extraction ($0.38–$0.42/mo) plus full-document screening agent tokens ($27.50–$30.25/mo, `[REQ-F-11], [REQ-F-15]`); no vector index costs apply anywhere in this architecture. **Flagged risk:** the high-end margin under the `[REQ-COST-01]` ceiling is only 1.8%; recommend validating actual token usage against AWS Cost Explorer shortly after production launch, since normal volume variance could approach the ceiling.
